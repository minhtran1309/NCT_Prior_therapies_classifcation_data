"""
Runner for GPT-5 via OpenRouter.

Saves results incrementally to output/gpt5.csv after EACH row so no
progress is lost if interrupted. Rerun to continue from where you left off.

Usage:
    conda activate oncogpt && python -m src.run_gpt5 [--num_rows N] [--dry-run]
"""
...
def run(num_rows=100, dry_run=False):
    ...
    if valid.sum() > 0:
        print(f"  {TAG} 📊 Accuracy: {correct.sum()}/{valid.sum()} = {correct.sum()/valid.sum():.2%}")
